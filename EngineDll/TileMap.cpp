		...
		for (size_t x = 0; x < _width; x++){
			for (size_t i = 0; i < _tileMapGrid.size(); i++)
			{
				if (!_tileMapGrid[i][y][x].isWalkable()){
					...
					if (_tileMapGrid[i][y][x].checkCollision(object) == Entity2D::CollisionHorizontalRight ||
						_tileMapGrid[i][y][x].checkCollision(object) == Entity2D::CollisionHorizontalLeft)
